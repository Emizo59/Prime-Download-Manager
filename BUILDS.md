# Build Log — Prime Download Manager

Sequential build numbers for every installer produced. The latest build is the one
currently published on the [Releases page](https://github.com/Emizo59/Prime-Download-Manager/releases).

| Build | Date | Version | Notes |
|------:|------|---------|-------|
| **006** | 2026-06-22 | 1.0 Beta 5 | Build-number-based update detection (fixes "up to date" when a newer build exists); version bumped to 1.0 Beta 5. Current published build. |
| 005 | 2026-06-22 | 1.0 Beta 4 | Removed "with AI" credit; amber "update pending" badge; in-app feedback/crash form (relay); Apple-style site animation; direct-download filename fix. Current published build. |
| **004** | 2026-06-22 | 1.0 Beta 4 | About dialog, in-app update check, crash reporting, EULA + publisher info. |
| 003 | 2026-06-21 | 1.0 Beta 4 | English pledge wording updated. |
| 002 | 2026-06-21 | 1.0 Beta 4 | Windows exe + installer + downloads folder renamed to "Prime Download Manager". |
| 001 | 2026-06-21 | 1.0 Beta 4 | First public release build. |
